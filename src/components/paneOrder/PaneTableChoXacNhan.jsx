...
let statusFilterChoXacNhan = () => {};
let nameFilterChoXacNhan = () => {};
let phoneFilterChoXacNhan = () => {};
let emailFilterChoXacNhan = () => {};
let orderIdFilter = () => {};

function PaneTableChoXacNhan(props) {
  ...
  const columnsOrderPane = [
    ...
    },
    {
      ...
      formatter: (status, storeDataOrder) => (
        <>
          {Number(status) === 1 ? <Badge color="warning">Chờ xác nhận</Badge> : null}
        </>
      ),
      filter: selectFilter({
        options: optionTrangThai,
        getFilter: (filter) => {
          statusFilterChoXacNhan = filter;
        },
        style: { display: "none" },
      }),
      headerClasses: "table-light",
  ...
  useEffect(() => {
    if (typeof statusFilterChoXacNhan == "function" && value.length > 0) {
      statusFilterChoXacNhan(params.status);
      nameFilterChoXacNhan(params.name);
      phoneFilterChoXacNhan(params.phone);
      emailFilterChoXacNhan(params.email);
...
export default PaneTableChoXacNhan;
